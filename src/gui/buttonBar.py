...
class ButtonBar(tk.Frame):
    def __init__(self, *arg, **kwargs):
        ...
        # Add buttons:
        self.dir_addButton = ttk.Button(
                                   dirGroup, 
                                   text = "Add",
                                   command = self._on_add_press)
        self.dir_addButton.pack(side = tk.LEFT, padx = BUTTONBAR_BUTTON_PADX)


        # Save buttton:
        self.dir_saveButton = ttk.Button(
                                    dirGroup, 
                                    text = "Save",
                                    command = self._on_save_press)
        self.dir_saveButton.pack(side = tk.LEFT, padx = BUTTONBAR_BUTTON_PADX)


        # Check button:
        self.dir_checkButton = ttk.Button(
                                     dirGroup, 
                                     text = "Check",
                                     command = self._on_check_press)
        self.dir_checkButton.pack(side = tk.LEFT, padx = BUTTONBAR_BUTTON_PADX)


        # Change button:
        self.dir_changeButton = ttk.Button(
                                      dirGroup, 
                                      text = "Change",
                                      command = self._on_change_press)
        self.dir_changeButton.pack(side = tk.LEFT, padx = BUTTONBAR_BUTTON_PADX)
        ...
        # Start button:
        self.back_startButton = ttk.Button(
                                      backGroup, 
                                      text = "Start",
                                      command = self._on_start_press)
        self.back_startButton.pack(side = tk.LEFT, padx = BUTTONBAR_BUTTON_PADX)

        # Clean button:
        self.back_cleanButton = ttk.Button(
                                      backGroup, 
                                      text = "Clean",
                                      command = self._on_clean_press)
        self.back_cleanButton.pack(side = tk.LEFT, padx = BUTTONBAR_BUTTON_PADX)

        # Cancel button:
        self.back_cancelButton = ttk.Button(
                                       backGroup, 
                                       text = "Cancel",
                                       command = self._on_cancel_press)
        self.back_cancelButton.pack(side = tk.LEFT, padx = BUTTONBAR_BUTTON_PADX)

        # Clear button:
        self.back_clearButton = ttk.Button(
                                      backGroup, 
                                      text = "Clear",
                                      command = self._on_clear_press)
        self.back_clearButton.pack(side = tk.LEFT, padx = BUTTONBAR_BUTTON_PADX)
